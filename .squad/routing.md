...
|-----------|----------|----------|
| Public API shape & package boundaries | Morpheus | Abstractions changes, package ownership, transport-agnostic API decisions |
| .NET library implementation | Trinity | Builders, DI registration, factories, processors, runtime behavior |
| Azure backend integration | Link | Event Hub, Service Bus, Storage Queue specifics, metadata mapping, Azure SDK behavior |
| Testing & review | Tank | Unit tests, CI failures, regressions, reviewer gate, edge cases |
| Docs & samples | Oracle | README updates, sample apps, onboarding guidance, usage examples |
| Code review | Morpheus | Review PRs, check API consistency, suggest safer package boundaries |
